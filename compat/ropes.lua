...
jumpdrive.register_after_jump(function(from_area, to_area)
	...
	local target_bottom_pos2 = vector.new(target_pos2.x, target_pos1.y, target_pos2.z)

	-- Top slice of the target area
	local target_top_pos1 = vector.new(target_pos1.x, target_pos2.y, target_pos1.z)
	...
	for _, rope_type_nodes in ipairs(rope_nodes) do
		...
		local ropes_hanging_out = core.find_nodes_in_area(
		...
		for _, pos in ipairs(ropes_hanging_out) do
			...
			local end_node = core.get_node(pos)
			core.swap_node(pos, {
			...
			local remainder_pos = vector.offset(pos, -delta_vector.x, -delta_vector.y - 1, -delta_vector.z)
			ropes.destroy_rope(remainder_pos, rope_type_nodes)
		end


		-- Look for ropes hanging into the jump area
		local ropes_hanging_in = core.find_nodes_in_area(
		...
		for _, pos in ipairs(ropes_hanging_in) do
			...
			local end_pos = vector.offset(pos, -delta_vector.x, -delta_vector.y + 1, -delta_vector.z)
			local end_node = core.get_node(end_pos)
